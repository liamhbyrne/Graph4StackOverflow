    ...
    def process(self):
        ...
        for _, question in tqdm(valid_questions.iterrows(), total=len(valid_questions)):
            ...
            for _, answer in answers_to_question.iterrows():
                label = torch.tensor(
                    [1 if answer["Score"] > 0 else 0], dtype=torch.long
                )
                ...
                log.info(
                    f"Average graph construction time: {sum(TIMES)/len(TIMES)} seconds. {TIMES}"
                )
...
if __name__ == "__main__":
    ...
    ds = UserGraphDataset(
        "../datav2/", db_address="../stackoverflow.db", skip_processing=False
    )
    data = ds.get(1078)
