...
class StaticGraphConstruction:
    tag_embedding_model = NextTagEmbeddingTrainer.load_model(
        "../models/tag-emb-7_5mil-50d-63653-3.pt",
        embedding_dim=50,
        vocab_size=63654,
        context_length=3,
    )
    module_embedding_model = ModuleEmbeddingTrainer.load_model(
        "../models/module-emb-1milx5-30d-49911.pt", embedding_dim=30, vocab_size=49911
    )
    ...
    def process_questions(self, questions: pd.DataFrame) -> torch.Tensor:
        ...
        (
            ...
            code_emb_batches,
            module_name_batches,
        ) = StaticGraphConstruction.post_embedding_builder(
    ...
    def process_modules(self):
        ...
        for module in self._known_modules:  # TODO: Map module name to its embedding
            yield StaticGraphConstruction.module_embedding_model.get_module_embedding(
                module
            )
    ...
    def construct(self, questions, answers, comments) -> HeteroData:
        ...
        print(
            sum(
                [
                    len(graph_out["question"].x),
                    len(graph_out["answer"].x),
                    len(graph_out["comment"].x),
                    len(graph_out["tag"].x),
                    len(graph_out["module"].x),
                ]
            )
        )
