...
class ModuleEmbeddingTrainer:
    ...
    def from_db(self, row_limit=100000, save_path: str = None):
        post_body_series = pd.read_sql_query(
            f"SELECT Body FROM Post WHERE (Tags LIKE '%python%') AND (Body LIKE '%import%')  LIMIT {row_limit}",
            self.db,
        )

        modules_series = post_body_series["Body"].apply(
            lambda html: [
                x.module
                for x in self.emb_builder.get_imports_via_regex(
        ...
        combinations = modules_series.apply(
    ...
    def train(self, train_size: int, epochs: int):
        ...
        losses = []
        # Model
        self.model = ModuleEmbedding(
            vocab_size=len(self.module_vocab), embedding_dim=self.emb_size
        ).to(self.device)
        # Optimizer
        ...
        self.module_to_ix = {
            module_name: i for i, module_name in enumerate(self.module_vocab)
        }
        ...
        for epoch in range(epochs):
            ...
            for mod_a, mod_b in tqdm(samples):
                mod_a_id = torch.tensor(self.module_to_ix[mod_a], dtype=torch.long).to(
                    self.device
                )
                ...
                loss = loss_function(
                    log_probs.flatten(),
                    torch.tensor(self.module_to_ix[mod_b], dtype=torch.long).to(
                        self.device
                    ),
                )
                loss.backward()
