...
class Baseline(torch.nn.Module):
    """
    Baseline model for upvote classification of StackOverflow answers
    """
...
if __name__ == "__main__":
    ...
    if IN_MEMORY_DATASET:
        train_dataset = UserGraphDatasetInMemory(
            root="../data", file_name_out="train-4175-qs.pt", question_ids=[]
        )
        test_dataset = UserGraphDatasetInMemory(
            root="../data", file_name_out="test-1790-qs.pt", question_ids=[]
        )
    else:
    ...
    if CROSS_VALIDATE:
        ...
        for i in range(len(folds)):
            ...
            train_fold_loader = DataLoader(
                train_fold, batch_size=TRAIN_BATCH_SIZE, sampler=sampler
            )
            test_fold_loader = DataLoader(test_fold, batch_size=TEST_BATCH_SIZE)
            # Model
            model = Baseline(hidden_channels=HIDDEN_CHANNELS, out_channels=2)
            model.to(device)  # To GPU if available

            if WARM_START_FILE is not None:
                model.load_state_dict(torch.load(WARM_START_FILE))
            ...
            criterion = torch.nn.CrossEntropyLoss(
                weight=fold_class_weights if USE_CLASS_WEIGHTS_LOSS else None
            )
    ...
    criterion = torch.nn.CrossEntropyLoss(
        weight=class_weights if USE_CLASS_WEIGHTS_LOSS else None
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # log for current epoch
        log.info(
            f'Epoch: {epoch:03d}, Loss {train_info["loss"]}, Train F1: {train_info["f1-score"]:.4f}, Test F1: {test_info["f1-score"]:.4f}'
        )
