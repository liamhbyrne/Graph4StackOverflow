...
if __name__ == "__main__":
    ...
    if IN_MEMORY_DATASET:
        train_dataset = UserGraphDatasetInMemory(
            root=ROOT, file_name_out=TRAIN_DATA_PATH, question_ids=[]
        )
        test_dataset = UserGraphDatasetInMemory(
            root=ROOT, file_name_out=TEST_DATA_PATH, question_ids=[]
        )
    else:
    ...
    if CROSS_VALIDATE:
        ...
        for i in range(len(folds)):
            ...
            # Model
            model = HeteroGNN(
                hidden_channels=HIDDEN_CHANNELS, out_channels=2, num_layers=NUM_LAYERS
            )
