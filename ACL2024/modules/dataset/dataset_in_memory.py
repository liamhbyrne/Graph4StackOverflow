...
class UserGraphDatasetInMemory(InMemoryDataset):
    def __init__(self, root, file_name_out: str, question_ids: List[int], transform=None, pre_transform=None, pre_filter=None):
...
def create_datasets_for_kfolds(folds, root):
    ...
    for i, fold_question_ids in enumerate(folds):
        yield UserGraphDatasetInMemory('../data/', f'fold-{i + 1}-{len(question_ids)}-qs.pt', fold_question_ids)
