...
class PostEmbedding(nn.Module):
    ...
    def __init__(self, batched=False):
        ...
        self._roberta_model = RobertaModel.from_pretrained(
    ...
    def to_bert_embedding(self, texts: List[str]) -> torch.tensor:
        encodings = self._roberta_tokenizer(
...
if __name__ == "__main__":
    ...
    print(
        [
            x.module
            for x in pe.get_imports_via_regex(
                BeautifulSoup("<code>import ast<\code>", "lxml")
            )
        ]
    )
    """TIME END"""
